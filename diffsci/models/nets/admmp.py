...
class ADMEncoderBlock(torch.nn.Module):
    def __init__(self,
                 channels_in: int,
                 channels_out: int,
                 channels_embed: int,
        ...
        self.norm1, self.norm2 = self.make_norm_layers(channels_in,
                                                       channels_out)
        self.conv1 = self.conv_fn(channels_in,
                                  channels_out,
                                  kernel_size=3,
                                  padding='same')
        ...
        # self.embed_linear = torch.nn.Sequential(
        #     torch.nn.Linear(channels_embed, 4*channels_embed),
        #     torch.nn.SiLU(),
        #     torch.nn.Linear(4*channels_embed, 4*channels_embed),
        #     torch.nn.SiLU(),
        #     torch.nn.Linear(4*channels_embed, 2*channels_out)
        # )

        self.dropout = torch.nn.Dropout(pdrop)

        if self.has_residual:
            self.convresidual = self.conv_fn(channels_in,
                                             channels_out,
                                             kernel_size=1)
        if self.has_downsample:
            self.downsample = self.downsample_fn(downsample_factor)

        if self.has_attn:
            self.attn = self.make_attn_layer()

    def forward(self, x, te):
        # x : (B, Cin, H, W) or (B, C, D, H, W)
        # In the following comments, we assume that x is (B, C, H, W)
        x1 = self.first_block(x)  # (B, Cout, Hout, Wout)
        te1, te2 = self.embed_block(te)  # (B, Cout, 1, 1), (B, Cout, 1, 1)
        x1t = x1*te1 + te2  # (B, Cout, H, W)
        x2 = self.second_block(x1t)  # (B, Cout, H, W)
        if self.has_residual:
            xs = self.residual_block(x)  # (B, Cout, H, W)
            x2 = x2 + xs
        if self.has_attn:
            x2 = self.attn(x2)
        return x2
    ...
    def downsample_fn(self):
        if self.dimension == 1:
            ...
            raise ValueError(f"Invalid dimension {self.dimension}")
        return downsample_cls

    def make_norm_layers(self, channels_in, channels_out):
        if self.first_norm == 'GroupLN':
            norm1 = torch.nn.GroupNorm(self.num_groups, channels_in)
        elif self.first_norm == 'GroupRMS':
            norm1 = commonlayers.GroupRMSNorm(self.num_groups, channels_in)
        else:
            raise ValueError(f"Invalid norm {self.first_norm}")
        if self.second_norm == 'GroupLN':
            norm2 = torch.nn.GroupNorm(self.num_groups, channels_out)
        elif self.second_norm == 'GroupRMS':
            norm2 = commonlayers.GroupRMSNorm(self.num_groups, channels_out)
        else:
...
class ADMDecoderBlock(torch.nn.Module):
    def __init__(self,
                 channels_in: int,
                 channels_out: int,
                 channels_embed: int,
    ...
    @property
    def has_skip(self):
        return self.channels_skip is not None

    @property
    def channels_in_modified(self):
        if self.has_skip and self.skip_integration_type == 'concat':
