...
from .utils import GeoGaussianFourierProjection
from .utils import GroupRMSNorm
...
class ResnetBlockB(torch.nn.Module):
    def __init__(self, input_channels,
        ...
        if date_embed_boll:
            self.dateblock = ResnetTimeBlock(time_embed_dim, output_channels)

        if geo_embed_boll:
            self.geoblock = ResnetTimeBlock(time_embed_dim, output_channels)
    ...
    def forward(self, x, te, de=None, ge=None):
...
class OUNet(torch.nn.Module):
    def __init__(self, model_channels,
        ...
        super().__init__()
        self.date_embed_boll = date_embed_boll
        self.geo_embed_boll = geo_embed_boll
        ...
        if geo_embed_boll:
            self.geo_emb = GeoGaussianFourierProjection(model_channels)
    ...
    def forward(self, x, t, dates=None, latlon=None):
        """
        Parameters
        ----------
        ...
        if latlon is not None:
            ge = self.geo_emb(latlon)  # (nbatch, m)
        else:
            ge = None
        ...
        y1 = self.resnet1b(
            self.resnet1a(y, te, de, ge), te, de, ge
        )  # (nbatch, m, w, h)
        ...
        y2 = self.resnet2b(
            self.resnet2a(y, te, de, ge), te, de, ge
        )  # (nbatch, 2*m, w//2, h//2)
        ...
        # The lower part
        y = self.resnet3b(
            self.resnet3a(y, te, de, ge), te, de, ge
        )  # (nbatch, 4*m, w//4, h//4)

        # y = y + self.attn(y)  # (nbatch, 4*m, w//4, h//4)
        yattn = self.resnetattnb(
                    self.attn(self.resnetattna(y, te, de, ge)), te, de, ge
        )  # (nbatch, 4*m, w//4, h//4)
        y = y + yattn

        y = self.resnet3d(
            self.resnet3c(y, te, de, ge), te, de, ge
        )  # (nbatch, 4*m, w//4, h//4)
        ...
        y = self.resnet4b(
            self.resnet4a(y, te, de, ge), te, de, ge
        )  # (nbatch, 2*m, w//2, h//2)

        # Upsampling back
        y = self.upsampler45(y) + y1  # (nbatch, m, w, h)
        y = self.resnet5b(self.resnet5a(y, te, de, ge), te, de, ge)
        # (nbatch, m, w,h)
    ...
    def resnet_block_fn(self,
        ...
        return ResnetBlockB(input_multiplier*self.model_channels,
                            ...
                            date_embed_boll=self.date_embed_boll,
                            geo_embed_boll=self.geo_embed_boll
                            )
...
class OUNetUncond(OUNet):
    ...
    def __init__(self, model_channels,
        ...
        super().__init__(model_channels,
                         ...
                         kernel_size=kernel_size,
                         dropout=dropout,
                         date_embed_boll=date_embed_boll,
                         geo_embed_boll=geo_embed_boll
                         )
...
class OUNetCond(OUNet):
    ...
    def forward(self, x, t, y):
        ...
        torch.Tensor of shape (B, channels, H, W)
        """
        if self.date_embed_boll:
            dates = y['dates']
        else:
            dates = None
        if self.geo_embed_boll:
            latlon = y['latlon']
        else:
            latlon = None
        if self.batimetry:
        ...
        # x : (nbatch, 1, w, h)
        # y : (nbatch, 1, w, h)
        x = torch.concatenate([x, y], axis=1).to(x.device)
        # (nbatch, 2, w, h)
        # print(f'PunetCOND input shape{x.shape}')
        return super().forward(x, t, dates, latlon)
