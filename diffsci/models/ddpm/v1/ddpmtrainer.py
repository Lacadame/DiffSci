...
class DDPMTrainer(object):
    ...
    def loss_fn(self, x, t, y=None):
        ...
        calpha = calpha[:, None, None, None]  # (nbatch, 1, 1, 1)
        lambd = lambd[:, None, None, None]  # (nbatch, 1, 1, 1)
        lambd = lambd  # Normalization
        ...
        if self.conditional:
            score = self.model(x_noised, t, y)  # (nbatch, 1, H, W)
        elif not self.conditional:
            score = self.model(x_noised, t)  # (nbatch, 1, H, W)
        # NOTE: we do mean in batch and then sum just to be dimension agnostic
        #       here
        # loss = (lambd*(score-noise)**2).mean(dim=0).sum()
        loss = (lambd*self.loss_metric(score, noise)).mean(dim=0).sum()
    ...
    def train(self, nepochs=20, on_notebook=False):
        ...
        on_notebook : bool
            Whether we are training on a notebook or not
        """
        ...
        for epoch in tqdm_epoch:
            ...
            for data in tqdm_step:
                ...
                if self.conditional:
                    ...
                    x = x.to(self.device)  # Move to device
                    y = None  # No y
                t = self.scheduler.sample(x.shape[0])  # Sample t
...
class CondDDPMTrainer(DDPMTrainer):
    ...
    def __init__(self, model, scheduler, train_dataloader,
        ...
        """
        A wrapper for DDPMTrainer that is conditional by default
        """
...
class UncondDDPMTrainer(DDPMTrainer):
    ...
    def __init__(self, model, scheduler, train_dataloader,
        ...
        """
        A wrapper for DDPMTrainer that is unconditional by default
        """
